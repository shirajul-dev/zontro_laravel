...
$watch('darkMode', value => localStorage.setItem('darkMode', JSON.stringify(value)))" :class="{ 'dark bg-gray-900': darkMode === false }">
    <!-- ===== Preloader Start ===== -->
    ...
    <div x-data="{ 
         ...
         class="fixed top-5 right-5 z-999999 flex flex-col gap-3">
        ...
        <template x-for="toast in toasts" :key="toast.id">
            <div x-transition:enter="transition ease-out duration-300"
                 ...
                 class="flex items-center gap-3 px-5 py-3 rounded-lg text-white shadow-xl min-w-[250px]">
    ...
    @if(session('success'))
        <div x-init="$nextTick(() => { $dispatch('toast', { type: 'success', message: '{{ session('success') }}' }) })"></div>
    @endif
    @if(session('error'))
        <div x-init="$nextTick(() => { $dispatch('toast', { type: 'error', message: '{{ session('error') }}' }) })"></div>
    @endif
