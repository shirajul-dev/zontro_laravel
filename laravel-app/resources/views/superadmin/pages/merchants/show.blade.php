        ...
        <div class="flex flex-col gap-5 lg:gap-7.5">
            ...
            <div class="kt-card relative pb-10">
                <div class="kt-card-content lg:pt-12">
                    ...
                    <div class="absolute top-5 end-5 flex gap-2">
                        <a href="{{ route('superadmin.merchants.edit', $merchant->a_id) }}"
                            class="kt-btn kt-btn-outline kt-btn-xs sm:kt-btn-sm">
                            <i class="ki-filled ki-pencil"></i> Edit
                        </a>
                        @if ($merchant->status == 'active')
                            <button onclick="openSuspendModal('{{ $merchant->a_id }}', '{{ $merchant->brand_name }}')"
                                class="kt-btn kt-btn-danger kt-btn-xs sm:kt-btn-sm">
                                <i class="ki-filled ki-slash"></i> Suspend
                            </button>
                        @else
                            <button onclick="openReactivateModal('{{ $merchant->a_id }}', '{{ $merchant->brand_name }}')"
                                class="kt-btn kt-btn-success kt-btn-xs sm:kt-btn-sm">
                    ...
                    <div class="flex justify-center mb-5">
                        <div class="size-16 lg:size-24 relative">
                            @if ($merchant->brand_logo)
                                <img class="rounded-full size-full object-contain p-1 border border-primary/20 bg-background"
                                    src="{{ asset($merchant->brand_logo) }}" />
                            @else
                                <div
                                    class="rounded-full size-full bg-primary/10 flex items-center justify-center border border-primary/20">
                                    <span
                                        class="text-3xl font-bold text-primary">{{ substr($merchant->brand_name ?? $merchant->full_name, 0, 1) }}</span>
                                </div>
                            @endif
                            <div
                                class="flex size-3 bg-{{ $merchant->status == 'active' ? 'green' : 'red' }}-500 rounded-full absolute bottom-0.5 end-1 border-2 border-background">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid justify-center gap-2 mb-8">
                        ...
                        <div
                            class="flex items-center justify-center gap-2 px-3 py-1 bg-muted/50 rounded-full border border-border">
                    ...
                    <div class="flex items-center justify-center flex-wrap gap-3 lg:gap-6">
                        <div
                            class="grid grid-cols-1 gap-1 border border-dashed border-border rounded-xl px-5 py-3 min-w-32 bg-muted/10">
                            <span class="text-mono text-xl font-bold text-foreground">0</span>
                            <span
                                class="text-muted-foreground text-xs font-semibold uppercase tracking-tight">Transactions</span>
                        </div>
                        <div
                            class="grid grid-cols-1 gap-1 border border-dashed border-border rounded-xl px-5 py-3 min-w-32 bg-muted/10">
                            <span class="text-mono text-xl font-bold text-foreground">0</span>
                            <span
                                class="text-muted-foreground text-xs font-semibold uppercase tracking-tight">Customers</span>
                        </div>
                        <div
                            class="grid grid-cols-1 gap-1 border border-dashed border-border rounded-xl px-5 py-3 min-w-32 bg-muted/10">
                        ...
                        <div
                            class="grid grid-cols-1 gap-1 border border-dashed border-primary/30 rounded-xl px-5 py-3 min-w-32 bg-primary/5">
                            <span
                                class="text-mono text-xl font-bold text-primary">{{ $merchant->plan_name ?? 'No Plan' }}</span>
                            <span class="text-primary/60 text-xs font-semibold uppercase tracking-tight">Current Plan</span>
                        </div>
                        <div
                            class="grid grid-cols-1 gap-1 border border-dashed border-{{ $merchant->status == 'active' ? 'primary' : 'danger' }} rounded-xl px-5 py-3 min-w-32 bg-{{ $merchant->status == 'active' ? 'primary' : 'danger' }}/5">
                            <span
                                class="text-mono text-xl font-bold text-{{ $merchant->status == 'active' ? 'primary' : 'danger' }}">{{ strtoupper($merchant->status) }}</span>
                            <span
                                class="text-{{ $merchant->status == 'active' ? 'primary' : 'danger' }}/60 text-xs font-semibold uppercase tracking-tight">Account
                                Status</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-7.5">
                ...
                <div class="flex flex-col gap-5 lg:gap-7.5">
                        ...
                        <div class="kt-card-content pt-4 pb-6">
                            <div class="flex flex-col gap-5 px-5 lg:px-7.5">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-bold text-foreground">{{ \Carbon\Carbon::parse($merchant->created_date)->format('M d, Y') }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm font-medium text-muted-foreground">Country:</span>
                                    <span
                                        class="text-sm font-bold text-foreground">{{ $merchant->brand_country ?? 'N/A' }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm font-medium text-muted-foreground">Branding ID:</span>
                                    <span
                                        class="text-sm font-bold text-foreground text-mono text-xs">{{ $merchant->brand_id }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="kt-card">
                        <div class="kt-card-header">
                            <h3 class="kt-card-title">Subscription Plan</h3>
                            <div class="kt-badge kt-badge-sm kt-badge-outline kt-badge-primary">
                                {{ $merchant->plan_name ?? 'N/A' }}
                            </div>
                        </div>
                        <div class="kt-card-content pt-4 pb-6 px-5 lg:px-7.5">
                            <div class="grid gap-4">
                                @php
                                    $planFeatures = json_decode($merchant->plan_features, true) ?? [];
                                    $allPossibleFeatures = [
                                        'invoices' => ['label' => 'Invoices & Billing', 'icon' => 'ki-bill'],
                                        'payment_links' => ['label' => 'Payment Links', 'icon' => 'ki-link'],
                                        'api_access' => ['label' => 'API & Webhooks', 'icon' => 'ki-code'],
                                        'settlements' => ['label' => 'Settlements', 'icon' => 'ki-wallet'],
                                        'multi_currency' => ['label' => 'Multi-Currency', 'icon' => 'ki-bitcoin'],
                                    ];
                                @endphp

                                @foreach ($allPossibleFeatures as $key => $data)
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <i
                                                class="ki-filled {{ $data['icon'] }} text-lg {{ $planFeatures[$key] ?? false ? 'text-primary' : 'text-muted-foreground/30' }}"></i>
                                            <span
                                                class="text-sm {{ $planFeatures[$key] ?? false ? 'text-foreground font-medium' : 'text-muted-foreground/50' }}">{{ $data['label'] }}</span>
                                        </div>
                                        @if ($planFeatures[$key] ?? false)
                                            <i class="ki-filled ki-check text-success text-xs"></i>
                                        @else
                                            <i class="ki-filled ki-cross text-muted-foreground/20 text-xs"></i>
                                        @endif
                                    </div>
                                @endforeach
                                ...
                                <div class="mt-4 pt-4 border-t border-dashed border-border">
                                    <a href="{{ route('superadmin.merchants.edit', $merchant->a_id) }}"
                                        class="kt-btn kt-btn-outline kt-btn-xs w-full">
                                        <i class="ki-filled ki-arrows-loop"></i> Change Plan
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="kt-card-content pt-4 pb-6 px-5 lg:px-7.5">
                            <div class="grid gap-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="size-9 rounded-lg bg-accent/60 flex items-center justify-center border border-border">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <a href="mailto:{{ $merchant->support_email }}"
                                            class="text-sm font-medium hover:text-primary">{{ $merchant->support_email ?? 'N/A' }}</a>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    <div
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <a href="{{ $merchant->support_website }}" target="_blank"
                                            class="text-sm font-medium hover:text-primary">{{ $merchant->support_website ?? 'N/A' }}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="p-5 rounded-xl border border-dashed border-border bg-muted/30">
                                        ...
                                        <div class="grid gap-2 text-sm">
                                            <div class="flex justify-between">
                                                ...
                                                <span
                                                    class="kt-badge kt-badge-sm kt-badge-outline kt-badge-warning">Pending</span>
                                            </div>
                                            <div class="flex justify-between">
                                                <span class="text-muted-foreground">Risk Level:</span>
                                                <span
                                                    class="kt-badge kt-badge-sm kt-badge-outline kt-badge-success">Low</span>
                                            </div>
                                            <div class="flex justify-between">
                    ...
                    <div class="kt-card">
                        ...
                        <div class="kt-card-content py-10 text-center">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <span class="text-muted-foreground font-medium">No recent activity found for this
                                    merchant.</span>
                            </div>
                        </div>
                    </div>
