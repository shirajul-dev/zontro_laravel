@php
    ...
    $canCreate = ($userRole === 'root' || ($permissions['brands']['create'] ?? false));
    $canEdit = ($userRole === 'root' || ($permissions['brands']['edit'] ?? false));
    $canDelete = ($userRole === 'root' || ($permissions['brands']['delete'] ?? false));
@endphp
        ...
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    @if($canCreate)
                    <span onclick="load_content('Brands','{{ $site_url }}{{ $path_admin }}/brands/create','nav-item-brands')">
                ...
                <div class="card">
                  ...
                  <div class="table-responsive">
                    <table class="table table-selectable card-table table-vcenter text-nowrap datatable">
                      <thead>
                        <tr>
                            <th class="w-1"><input class="form-check-input m-0 align-middle select-all" type="checkbox" aria-label="Select all"></th>
                            <th>Brand</th>
                            <th>Created Date</th>
                            <th></th>
                        </tr>
...
<script data-cfasync="false">
    $('.model-bulkAction-btn').click(function () {
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ $site_url }}{{ $path_admin }}/dashboard', {{-- Keep bulk actions on legacy handler for now or migrate them too --}}
    ...
    function initCheckboxTable() {
        ...
        rowCheckboxes.forEach(cb => {
            cb.addEventListener('change', () => {
                if(selectAll) selectAll.checked = rowCheckboxes.length === document.querySelectorAll('.rowCheckbox:checked').length;
                updateSelection();
            });
        });
    }
    ...
    function load_data_list(page = 1){
        ...
        $(".table-data-list").html('<tr><td colspan="4" class="text-center text-muted"><div class="spinner-border text-primary" style="margin: 50px;"></div></td></tr>');

        $.ajax({
            type: 'POST',
            url: '{{ route("admin.brands.list.ajax") }}',
            data: { _token: '{{ csrf_token() }}', search_input: search_input, show_limit: show_limit, page: page, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
