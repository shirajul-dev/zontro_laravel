<?php
    ...
    if ($ref === null) {
        ...
        if($response_gateway['status'] == false){
            ...
            $moduleSlug = (string) ($response_gateway['response'][0]['slug'] ?? '');
            $moduleBase = realpath(base_path('pp-content/pp-modules/pp-gateways'));
            $classFile = null;

            if ($moduleBase !== false && preg_match('/^[a-z0-9][a-z0-9-]*$/', $moduleSlug)) {
                $candidate = $moduleBase . DIRECTORY_SEPARATOR . $moduleSlug . DIRECTORY_SEPARATOR . 'class.php';
                $resolved = realpath($candidate);

                if ($resolved !== false && is_file($resolved) && str_starts_with($resolved, $moduleBase . DIRECTORY_SEPARATOR)) {
                    $classFile = $resolved;
                }
            }
