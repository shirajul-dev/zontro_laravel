    ...
    <div class="container container-tight py-4">
        <div class="card">
            <div class="card-body text-center">
                <div class="mb-4 mt-4">
                    <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" fill="currentColor" class="bi bi-{{ $currentStatus['icon'] }} text-{{ $currentStatus['color'] }}" viewBox="0 0 16 16">
                        @switch($status)
                            @case('completed')
                ...
                <p class="text-muted mb-4">
                    @switch($status)
                        @case('completed') {{ $lang['change_status_completed'] }} @break
                        @case('pending')   {{ $lang['change_status_pending'] }} @break
                        @case('refunded')  {{ $lang['change_status_refunded'] }} @break
                        @case('canceled')  {{ $lang['change_status_cancled'] }} @break
                    @endswitch
                </p>
                ...
                <div class="table-responsive mb-4 {{ $status == 'canceled' ? 'd-none' : '' }}">
                    <table class="table table-bordered">
                        <tbody>
                            ...
                            <tr>
                                <th>{{ $lang['amount'] }}</th>
                                <td>{{ number_format($transaction['amount'] ?? 0, 2) }} {{ $transaction['currency'] }}</td>
                            </tr>
                            <tr>
                                <th>{{ $lang['status'] }}</th>
                ...
                <div class="mb-3">
                    @if($transaction['return_url'] != '--' && !empty($transaction['return_url']))
                        <a href="{{ $transaction['return_url'] }}" class="btn btn-primary">{{ $lang['go_to_site'] }}</a>
                    @endif
                    @if(in_array($status, ['completed', 'pending', 'refunded']))
                        <a href="?receipt" class="btn btn-success">{{ $lang['download_receipt'] }}</a>
                    @endif
                </div>
            </div>
        </div>
