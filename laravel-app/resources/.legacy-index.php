<?php
    ...
    $pp_functions_path = base_path('app/Support/zp-functions.php');
    if(file_exists($pp_functions_path)){
        if (isset($pp_functions_loaded)) {

        }else{
            require $pp_functions_path;

            if (isset($pp_functions_loaded)) {

            }else{
                if(file_exists(base_path('pp-404.php'))){
                    http_response_code(404);
                    require base_path('pp-404.php');
                    exit();
                }else{
    ...
    $pp_adapter_path = base_path('pp-content/pp-include/pp-adapter.php');
    if(file_exists($pp_adapter_path)){
        require_once $pp_adapter_path;
    }
    ...
    /*
    |--------------------------------------------------------------------------
    | Global Action Handler (action-v2)
    |--------------------------------------------------------------------------
    */
    if (isset($_POST['action-v2'])) {
        global $db_prefix, $site_url, $path_payment, $path_invoice;
        ...
        $action = (string) $_POST['action-v2'];

        if ($action === 'invoice') {
            $itemId = (string) ($_POST['itemid'] ?? $_POST['invoice_id'] ?? $_POST['iid'] ?? '');
            if ($itemId === '') {
                echo json_encode(['status' => 'false', 'title' => 'Invalid Request', 'message' => 'Invoice ID is missing.']);
                exit;
            }
            ...
            if ($response_invoice['status'] !== true) {
                echo json_encode(['status' => 'false', 'title' => 'Invalid Invoice', 'message' => 'Invoice not found.']);
                exit;
            }
            $invoiceRow = $response_invoice['response'][0];

            if ($invoiceRow['status'] === 'paid') {
                echo json_encode(['status' => 'false', 'title' => 'Invoice Paid', 'message' => 'This invoice has already been paid.']);
                exit;
            }

            // Calculate Total
            $total = '0';
            $params_items = [':invoice_id' => $itemId, ':brand_id' => $invoiceRow['brand_id']];
            $response_items = json_decode(getData($db_prefix . 'invoice_items', 'WHERE invoice_id = :invoice_id AND brand_id = :brand_id', '* FROM', $params_items), true);

            if ($response_items['status'] === true) {
                foreach ($response_items['response'] as $item) {
                    ...
                    $itemCost = money_mul($amount, $quantity);
                    $itemTotalCost = money_sub($itemCost, $discount);
                    $vatAmount = money_div(money_mul($itemTotalCost, $vat), '100');
                    $itemTotalCostWithVat = money_add($itemTotalCost, $vatAmount);
                    $total = money_add($total, $itemTotalCostWithVat);
                }
            }
            $total = money_add($total, (string) ($invoiceRow['shipping'] ?? '0'));

            $payment_id = generateItemID(27, 27);
            $columns = ['brand_id', 'ref', 'customer_info', 'amount', 'currency', 'source', 'source_info', 'created_date', 'updated_date'];
            $values = [
                $invoiceRow['brand_id'],
                $payment_id,
                $invoiceRow['customer_info'],
                money_sanitize($total),
                $invoiceRow['currency'],
                'invoice',
                json_encode(['type' => 'invoice', 'invoice_id' => $itemId]),
                getCurrentDatetime('Y-m-d H:i:s'),
                getCurrentDatetime('Y-m-d H:i:s')
            ];

            if (insertData($db_prefix . 'transaction', $columns, $values)) {
                echo json_encode(['status' => 'true', 'redirect' => $site_url . '/' . $path_payment . '/' . $payment_id]);
            } else {
                echo json_encode(['status' => 'false', 'title' => 'Transaction Failed', 'message' => 'Could not create payment session.']);
            }
            exit;
        }

        if ($action === 'payment-link-default' || $action === 'payment-link') {
            $itemId = (string) ($_POST['itemid'] ?? '');
            $fullName = (string) ($_POST['full-name'] ?? '');
            $email = (string) ($_POST['email-address'] ?? '');
            $mobile = (string) ($_POST['mobile-number'] ?? '');
            $amountInput = (string) ($_POST['amount'] ?? '0');
            ...
            // Resolve brand and currency
            $params = [':brand_id' => ($action === 'payment-link-default' ? $itemId : '')];
            if ($action === 'payment-link-default') {
                ...
                if ($response_brand['status'] !== true) {
                    echo json_encode(['status' => 'false', 'title' => 'Invalid Brand', 'message' => 'Brand not found.']);
                    exit;
                }
                $brandRow = $response_brand['response'][0];
                $brandId = $brandRow['brand_id'];
                $currency = (($v = get_env('payment-link-default-currency', $brandId)) && $v !== '--') ? $v : $brandRow['currency_code'];
                $amount = money_sanitize($amountInput);
            } else {
                // Standard payment link
                $params = [':ref' => $itemId];
                $response_link = json_decode(getData($db_prefix . 'payment_link', 'WHERE ref = :ref', '* FROM', $params), true);
                if ($response_link['status'] !== true) {
                    echo json_encode(['status' => 'false', 'title' => 'Invalid Link', 'message' => 'Payment link not found.']);
                    exit;
                }
                $linkRow = (isset($response_link['status_true']) && $response_link['status_true']) ? $response_link['response'][0] : null; // Some versions use status_true
                if (!$linkRow) $linkRow = $response_link['response'][0];

                $brandId = $linkRow['brand_id'];
            ...
            $columns = ['brand_id', 'ref', 'customer_info', 'amount', 'currency', 'source', 'created_date', 'updated_date'];
            $values = [$brandId, $payment_id, $customerInfoJson, $amount, $currency, $action, getCurrentDatetime('Y-m-d H:i:s'), getCurrentDatetime('Y-m-d H:i:s')];
            ...
            echo json_encode([
                ...
                'redirect' => $site_url . '/' . $path_payment . '/' . $payment_id
            ]);
            exit;
        }
        ...
        if ($action === 'transaction-verify') {
            ...
            $sender_key = '';
            $sender_type = '';
            if (file_exists(app_path('Modules/gateways/' . $gateway['slug'] . '/class.php'))) {
                require_once app_path('Modules/gateways/' . $gateway['slug'] . '/class.php');
    ...
    */
    ...
    if(!file_exists(base_path('.maintenance'))){
        if(file_exists(base_path('pp-config.php')) || defined('LARAVEL_START')){
            if (isset($requriemntnoneedchecked) && $requriemntnoneedchecked === true) {
                switch ($route) {
                    ...
                    case 'forgot':
                    case '2fa':
                        if(file_exists(base_path('pp-content/pp-admin/'.$route.'.php'))){
                            require base_path('pp-content/pp-admin/'.$route.'.php');
                        }else{
                            if(file_exists(base_path('pp-404.php'))){
                                http_response_code(404);
                                require base_path('pp-404.php');
                    ...
                    case 'ipn':
                        ...
                        if($response_gateway['status'] == false){
                            ...
                            if($response_brand['status'] == true){
                                ...
                                if(file_exists(app_path('Modules/gateways/'.$response_gateway['response'][0]['slug'].'/class.php'))){
                                    require_once app_path('Modules/gateways/'.$response_gateway['response'][0]['slug'].'/class.php');
                    ...
                    case $path_payment:
                        ...
                        if($response_transaction['status'] == true){
                            ...
                            $response_brand = json_decode(getData($db_prefix.'brands','WHERE brand_id = :brand_id', '* FROM', $params),true);
                            if($response_brand['status'] == true){
                                if(file_exists(resource_path('views/theme/' . $response_brand['response'][0]['theme'] . '/class.php'))){
                                    require_once resource_path('views/theme/' . $response_brand['response'][0]['theme'] . '/class.php');
                    ...
                    case $path_admin:
                        ...
                        if(file_exists(base_path() . '/pp-content/pp-admin/index.php')){
                            require base_path() . '/pp-content/pp-admin/index.php';
                        }else{
                            if(file_exists(base_path() . '/pp-404.php')){
                                http_response_code(404);
                                require base_path() . '/pp-404.php';
                    ...
                    case $path_cron:
                        if($param1 == ""){
                            ...
                            if(escape_string($param1) == get_env('cron-job')){
                                $lockFile = base_path() . '/pp-media/storage/cron.lock';
                                $maxLockTime = 60 * 10;
                ...
                if(file_exists(base_path() . '/pp-requirement.php')){
                    require base_path() . '/pp-requirement.php';
                }else{
                    if(file_exists(base_path() . '/pp-404.php')){
                        http_response_code(404);
                        require base_path() . '/pp-404.php';
            ...
            if(file_exists(base_path() . '/pp-content/pp-install/index.php')){
                require base_path() . '/pp-content/pp-install/index.php';
            }else{
                if(file_exists(base_path() . '/pp-404.php')){
                    http_response_code(404);
                    require base_path() . '/pp-404.php';
        ...
        if(file_exists(base_path() . '/pp-maintenance.php')){
            require base_path() . '/pp-maintenance.php';
        }else{
            if(file_exists(base_path() . '/pp-404.php')){
                http_response_code(404);
                require base_path() . '/pp-404.php';
