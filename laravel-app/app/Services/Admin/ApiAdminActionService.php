<?php

namespace App\Services\Admin;

use App\Models\PpApi;

class ApiAdminActionService
{
    public function create(array $input, string $brandId): array
    {
        $apiName = trim((string) ($input['api_name'] ?? ''));
        $apiExpiryDate = trim((string) ($input['apiExpiryDate'] ?? ''));
        $apiStatus = trim((string) ($input['api_status'] ?? ''));
        $scopes = $input['scopes'] ?? [];
        $scopes = is_array($scopes) ? $scopes : [];

        if ($apiName === '') {
            return [
                'status' => 'false',
                'title' => 'Incomplete Information',
                'message' => 'Please fill in all required fields before proceeding.',
            ];
        }

        $exists = PpApi::query()
            ->where('brand_id', $brandId)
            ->where('name', $apiName)
            ->exists();

        if ($exists) {
            return [
                'status' => 'false',
                'title' => 'API Name Already Exists',
                'message' => 'This API name is already in use. Please choose a different name.',
            ];
        }

        if ($apiExpiryDate !== '') {
            if (function_exists('dateformat') && !dateformat($apiExpiryDate, 'Y-m-d')) {
                return [
                    'status' => 'false',
                    'title' => 'Invalid expiry date format',
                    'message' => 'Please enter the expiry date in the correct format (DD/MM/YYYY).',
                ];
            }
        } else {
            $apiExpiryDate = '--';
        }

        PpApi::query()->create([
            'brand_id' => $brandId,
            'name' => $apiName,
            'api_key' => bin2hex(random_bytes(25)),
            'expired_date' => $apiExpiryDate,
            'status' => $apiStatus,
            'api_scopes' => json_encode($scopes),
            'created_date' => now()->format('Y-m-d H:i:s'),
            'updated_date' => now()->format('Y-m-d H:i:s'),
        ]);

        return [
            'status' => 'true',
            'title' => 'Api Created',
            'message' => 'The api has been created successfully.',
        ];
    }

    public function list(array $input, string $brandId, string $brandTimezone): array
    {
        $searchInput = trim((string) ($input['search_input'] ?? ''));
        $filterStatus = trim((string) ($input['filter_status'] ?? ''));
        $filterStart = trim((string) ($input['filter_start'] ?? ''));
        $filterEnd = trim((string) ($input['filter_end'] ?? ''));

        $page = max(1, (int) ($input['page'] ?? 1));
        $rawShowLimit = (string) ($input['show_limit'] ?? '');
        $showLimit = $rawShowLimit === '' ? 999999 : (int) $rawShowLimit;
        if ($showLimit <= 0) {
            $showLimit = 8;
        }

        $query = PpApi::query()->where('brand_id', $brandId);

        if ($filterStart !== '') {
            $query->where('created_date', '>=', $filterStart . ' 00:00:00');
        }

        if ($filterEnd !== '') {
            $query->where('created_date', '<=', $filterEnd . ' 23:59:59');
        }

        if ($filterStatus !== '') {
            $query->where('status', $filterStatus);
        }

        if ($searchInput !== '') {
            $query->where('name', 'like', "%{$searchInput}%");
        }

        $totalRecords = (clone $query)->count();
        $offset = ($page - 1) * $showLimit;

        $rows = (clone $query)
            ->orderByDesc('id')
            ->offset($offset)
            ->limit($showLimit)
            ->get();

        if ($rows->isEmpty()) {
            return [
                'status' => 'false',
                'title' => 'Nothing Here Yet',
                'message' => 'No data is available at the moment.',
            ];
        }

        $timezone = $brandTimezone === '' || $brandTimezone === '--' ? 'Asia/Dhaka' : $brandTimezone;

        $response = $rows->map(function (PpApi $row) use ($timezone): array {
            $status = (string) $row->status;
            if ((string) $row->expired_date !== '--') {
                if ($this->isApiExpired((string) $row->expired_date)) {
                    $status = 'expired';
                }
            }

            return [
                'id' => (int) $row->id,
                'name' => (string) $row->name,
                'api_key' => (string) $row->api_key,
                'expired_date' => (string) $row->expired_date,
                'status' => $status,
                'created_date' => convertUTCtoUserTZ((string) $row->created_date, $timezone, 'M d, Y h:i A'),
                'updated_date' => convertUTCtoUserTZ((string) $row->updated_date, $timezone, 'M d, Y h:i A'),
            ];
        })->values()->all();

        $totalPages = max(1, (int) ceil($totalRecords / $showLimit));
        $start = $offset + 1;
        $end = min($offset + $showLimit, $totalRecords);

        return [
            'status' => 'true',
            'response' => $response,
            'datatableInfo' => "Showing <strong>{$start} to {$end}</strong> of <strong>{$totalRecords} entries</strong>",
            'pagination' => $this->buildPaginationHtml($page, $totalPages),
        ];
    }

    public function infoById(string $itemId, string $brandId): array
    {
        $row = PpApi::query()
            ->where('id', $itemId)
            ->where('brand_id', $brandId)
            ->first();

        if ($row === null) {
            return [
                'status' => 'false',
                'title' => 'Request Failed',
                'message' => 'Invalid request',
            ];
        }

        $scopes = json_decode((string) $row->api_scopes, true);
        $scopes = is_array($scopes) ? $scopes : [];

        return [
            'status' => 'true',
            'name' => (string) $row->name,
            'expired_date' => (string) $row->expired_date,
            'api_scopes' => $scopes,
            'astatus' => (string) $row->status,
        ];
    }

    public function bulkAction(string $actionId, array $selectedIds, string $brandId, bool $canDelete, bool $canEdit): array
    {
        if ($selectedIds === []) {
            return [
                'status' => 'false',
                'title' => 'Api Key Failed',
                'message' => 'No api selected.',
            ];
        }

        $ids = array_values(array_filter(array_map(static fn ($id) => (string) $id, $selectedIds), static fn ($id) => $id !== ''));
        if ($ids === []) {
            return [
                'status' => 'false',
                'title' => 'Api Key Failed',
                'message' => 'No api selected.',
            ];
        }

        foreach ($ids as $itemId) {
            $row = PpApi::query()
                ->where('id', $itemId)
                ->where('brand_id', $brandId)
                ->first();

            if ($row === null) {
                continue;
            }

            if ($actionId === 'deleted' && $canDelete) {
                $row->delete();
                continue;
            }

            if ($actionId === 'activated' && $canEdit) {
                $row->update([
                    'status' => 'active',
                    'updated_date' => now()->format('Y-m-d H:i:s'),
                ]);
                continue;
            }

            if ($actionId === 'inactivated' && $canEdit) {
                $row->update([
                    'status' => 'inactive',
                    'updated_date' => now()->format('Y-m-d H:i:s'),
                ]);
            }
        }

        return [
            'status' => 'true',
            'title' => 'Api Key ' . $actionId,
            'message' => 'The selected api key have been ' . $actionId . ' successfully.',
        ];
    }

    public function delete(string $itemId, string $brandId): array
    {
        PpApi::query()
            ->where('id', $itemId)
            ->where('brand_id', $brandId)
            ->delete();

        return [
            'status' => 'true',
            'title' => 'Api Key Deleted',
            'message' => 'The selected api key have been deleted successfully.',
        ];
    }

    public function edit(array $input, string $brandId): array
    {
        $apiId = trim((string) ($input['api_id'] ?? ''));
        $apiName = trim((string) ($input['api_name'] ?? ''));
        $apiExpiryDate = trim((string) ($input['apiExpiryDate'] ?? ''));
        $apiStatus = trim((string) ($input['api_status'] ?? ''));
        $scopes = $input['scopes'] ?? [];
        $scopes = is_array($scopes) ? $scopes : [];

        if ($apiName === '' || $apiStatus === '') {
            return [
                'status' => 'false',
                'title' => 'Incomplete Information',
                'message' => 'Please fill in all required fields before proceeding.',
            ];
        }

        $row = PpApi::query()
            ->where('brand_id', $brandId)
            ->where('id', $apiId)
            ->first();

        if ($row === null) {
            return [
                'status' => 'false',
                'title' => 'Request Failed',
                'message' => 'Invalid request',
            ];
        }

        $duplicate = PpApi::query()
            ->where('brand_id', $brandId)
            ->where('name', $apiName)
            ->where('id', '!=', $apiId)
            ->exists();

        if ($duplicate) {
            return [
                'status' => 'false',
                'title' => 'API Name Already Exists',
                'message' => 'This API name is already in use. Please choose a different name.',
            ];
        }

        if ($apiExpiryDate !== '') {
            if (function_exists('dateformat') && !dateformat($apiExpiryDate, 'Y-m-d')) {
                return [
                    'status' => 'false',
                    'title' => 'Invalid expiry date format',
                    'message' => 'Please enter the expiry date in the correct format (DD/MM/YYYY).',
                ];
            }
        } else {
            $apiExpiryDate = '--';
        }

        $row->update([
            'name' => $apiName,
            'expired_date' => $apiExpiryDate,
            'status' => $apiStatus,
            'api_scopes' => json_encode($scopes),
            'updated_date' => now()->format('Y-m-d H:i:s'),
        ]);

        return [
            'status' => 'true',
            'title' => 'Api Updated',
            'message' => 'The api has been updated successfully.',
        ];
    }

    private function isApiExpired(string $expiredDate): bool
    {
        if (function_exists('isExpired')) {
            return (bool) isExpired($expiredDate);
        }

        $ts = strtotime($expiredDate);
        if ($ts === false) {
            return false;
        }

        return $ts < strtotime(date('Y-m-d'));
    }

    private function buildPaginationHtml(int $page, int $totalPages): string
    {
        $html = '<ul class="pagination m-0 ms-auto">';

        $html .= '<li class="page-item' . ($page <= 1 ? ' disabled' : '') . '">'
            . '<button class="page-link" ' . ($page > 1 ? 'data-page="' . ($page - 1) . '"' : '') . '>'
            . '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-1">'
            . '<path d="M15 6l-6 6l6 6"></path>'
            . '</svg>'
            . '</button>'
            . '</li>';

        for ($i = 1; $i <= $totalPages; $i++) {
            $html .= '<li class="page-item' . ($i === $page ? ' active' : '') . '">'
                . '<button class="page-link" data-page="' . $i . '">' . $i . '</button>'
                . '</li>';
        }

        $html .= '<li class="page-item' . ($page >= $totalPages ? ' disabled' : '') . '">'
            . '<button class="page-link" ' . ($page < $totalPages ? 'data-page="' . ($page + 1) . '"' : '') . '>'
            . '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-1">'
            . '<path d="M9 6l6 6l-6 6"></path>'
            . '</svg>'
            . '</button>'
            . '</li>';

        $html .= '</ul>';

        return $html;
    }
}
