...
use App\Models\ZpGateway;
use App\Models\ZpGatewayParameter;
use Illuminate\Support\Str;

class GatewayMerchantActionService
{
    ...
    public function list(array $input, string $brandId): array
    {
        ...
        $query = ZpGateway::query()->where('brand_id', $brandId);
        ...
        if ($filterStart !== '') {
            $query->where('created_at', '>=', $filterStart . ' 00:00:00');
        }

        if ($filterEnd !== '') {
            $query->where('created_at', '<=', $filterEnd . ' 23:59:59');
        }
        ...
        $response = $rows->map(static function (ZpGateway $row): array {
            return [
    ...
    {
        $exists = ZpGateway::query()
            ->where('gateway_id', $itemId)
            ->where('brand_id', $brandId)
            ->exists();

        if ($exists) {
            ZpGateway::query()->where('gateway_id', $itemId)->delete();
            ZpGatewayParameter::query()->where('gateway_id', $itemId)->delete();
        }

        return [
            'status' => 'true',
            'title' => 'Gateway Deleted',
            'message' => 'The selected gateway has been deleted successfully.',
        ];
    }
    ...
    public function createGatewaySetting(array $postData, array $filesData, string $brandId, string $siteUrl): array
    {
        ...
        if ($logoFile instanceof \Illuminate\Http\UploadedFile) {
            ...
            if ($logoFile->getSize() <= $maxSize) {
                ...
                if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true)) {
                    $filename = strtolower(Str::random(10) . '_' . time() . '.' . $ext);
        ...
        foreach ($configData as $optName => $optVal) {
            $val = (string) $optVal;
            ZpGatewayParameter::query()->create([
                'brand_id' => $brandId,
                'gateway_id' => $gatewayId,
                'option_name' => $optName,
                'value' => $val === '' ? '--' : $val,
    ...
    public function updateGatewaySetting(array $postData, array $filesData, string $brandId, string $siteUrl): array
    {
        ...
        $gateway = ZpGateway::query()->where('gateway_id', $gatewayId)->where('brand_id', $brandId)->first();
        if ($gateway === null) {
            return [
                'status' => 'false',
                'title' => 'Request Failed',
        ...
        foreach ($configData as $optName => $optVal) {
            $val = (string) $optVal;
            $param = ZpGatewayParameter::query()
